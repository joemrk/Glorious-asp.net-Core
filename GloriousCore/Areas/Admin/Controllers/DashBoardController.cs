    ...
    public class DashBoardController : Controller
    {
        ...
        public IActionResult EditProduct(int id)
        {
            ...
            model = new ProductVM(dto)
            {
                ...
                Materials = new SelectList(db.Materials.ToList(), "Id", "Name")
            };
        ...
        [HttpPost]
        public async void SaveGallaryImages(int id, List<IFormFile> file) //не работает при создании товара т.к. у товора до создания нету ИД
        {
            foreach (var item in file)
            {
                if (item != null && item.Length > 0)
                {
                    ...
                    using (var stream = new MemoryStream())
                    {
                        ...
                        dto.Img = stream.ToArray();
                        dto.ImgType = item.ContentType;
                    }

                    //dto.ProductId = id;
                    //dto.ImgType = item.ContentType;
                    //dto.Img = new byte[item.ContentLength];
